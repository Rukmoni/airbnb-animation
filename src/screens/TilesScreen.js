...
class TilesScreen extends React.Component{
  ...
  render() {
    ...
    const {title, navigation,rootNavigation} = this.props;
    ...
    return (
      <View style={styles.flex} onLayout={this.onLayout}>
        {!navigation ? <NavBar title={title} icon='cart' onClickCart={this.onClickCart}/> : undefined}
        <FlatList
  ...
  onClickCart=()=>{
    ...
    const {navigation,root} = this.props;
    if (navigation) {
  ...
  onPressItem = (hero: Hero) => {
    ...
    if (navigation) {
     ...
     // this.props.rootNavigation.navigate("Details",{hero: sharedElements})
      Router.push(<Details {...props} />, {
        sharedElements,
        transitionConfig,
      });
    }
...
export default connect(mapStateToProps, bindAction)(TilesScreen);